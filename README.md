...
- **Anonymous entity resolution** — builds progressive identity (L0→L5) from IP+UA → TLS → HTTP/2 → client-side JS → behavioural patterns. Merge/split/rewind operations backed by immutable snapshots. Rotation creates a trail of near-miss cosine neighbours that get linked back to the same actor
...
- **API abuse** — no document request → no sequence context → full deferred detector stack always runs; machine-speed timing detected regardless of IP
